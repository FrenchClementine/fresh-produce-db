-- Simple fix - recreate the view with better fallback logic
...
SELECT
    ...
    CASE
        WHEN p.sold_by::text ILIKE '%kg%' OR p.sold_by = 'kg' THEN
            COALESCE(pps.weight_per_pallet, pps.boxes_per_pallet * pps.weight_per_box, 0)
        WHEN p.sold_by::text ILIKE '%box%' OR p.sold_by = 'box' THEN
