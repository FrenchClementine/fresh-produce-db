...
export function MyTransporterRoutesList() {
  const [searchTerm, setSearchTerm] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [editingRoute, setEditingRoute] = useState<string | null>(null)

  const { data: allRoutes, isLoading, error } = useTransporterRoutes()
  const { data: currentStaff } = useCurrentStaffMember()
  const deleteRoute = useDeleteTransporterRoute()

  // Filter routes to only show those for transporters assigned to current staff
    ...
    return (
      <Card>
        <CardContent className="p-6">
          <p className="text-muted-foreground">You must be logged in as a staff member to view your routes.</p>
        </CardContent>
      </Card>
    )
  }
  ...
  if (error) {
    return (
      <Card>
        <CardContent className="p-6">
          <p className="text-red-600">Error loading your routes. Please try again.</p>
        </CardContent>
      </Card>
    )
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Your Transporter Routes
            </CardTitle>
            <CardDescription>
              Manage hub-to-hub transportation services for your transporters
            </CardDescription>
          </div>
          <Dialog open={showForm} onOpenChange={setShowForm}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new hub-to-hub transportation service for your transporters
                </DialogDescription>
              </DialogHeader>
        ...
        <div className="space-y-4">
                ...
                {filteredRoutes.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                      {routes.length === 0
                        ? 'No routes found for your transporters. Add routes to your assigned transporters to get started.'
          ...
          {filteredRoutes.length > 0 && (
            <div className="text-sm text-muted-foreground">
              Showing {filteredRoutes.length} of {routes?.length} routes for your transporters
            </div>
          )}
        </div>
      </CardContent>
